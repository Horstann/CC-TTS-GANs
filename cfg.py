...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help='number of fake & real samples to be evaluated by discriminator for each identical condition')
    parser.add_argument(
        '-gen_ss',
        '--gen_sample_size',
        type=int,
        default=12,
        help='number of fake samples to be generated by generator for each identical condition; must be <= gen_ps')
    parser.add_argument(
        '--seq_len',
        type=int,
        default=42,
        help='sequence length of each price path to be generated')
    parser.add_argument(
        '--patch_size',
        type=int,
        default=6,
        help='size for each patch that seq_len will be divided to; must be a factor of seq_len')
    parser.add_argument(
        '-g_emb_size',
        '--gen_emb_size',
        type=int,
        default=10,
        help='embedding size for each time point for Generator, transformed from (latent_dim,) to (seq_len, gen_emb_size))')
    parser.add_argument(
        '-d_emb_size',
        '--dis_emb_size',
        type=int,
        default=10,
        help='embedding size for Discriminator, transformed from (num_patches, patch_size) to (num_patches, emb_size)')
    parser.add_argument(
        '--m2x_term_weight',
        type=float,
        default=1e-02,
        help="weight for the 'm2x' loss term (in functions.py)")
    parser.add_argument(
        '--var_term_weight',
        type=float,
        default=1e-3,
        help="weight for the 'var' loss term (in functions.py)")
    parser.add_argument(
        '--g_lr',
        type=float,
        default=0.0002,
