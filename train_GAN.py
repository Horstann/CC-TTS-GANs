...
#                 raise NotImplementedError('{} unknown inital type'.format(args.init_type))
    ...
    # init kernel_sigma and kappa
    if args.kernel_sigma is None:
        kernel_sigma = 1.06*np.std(train_set.X_train, axis=0) * (len(train_set.X_train))**(-1/5)
        args.kernel_sigma = kernel_sigma[0][0]
    ...
    if args.load_path:
        ...
        # Rename checkpoint attributes if necessary
        old_folder = checkpoint['path_helper']['prefix'].split('/')[-1]
        new_folder = re.findall(r'logs/(.+)/Model/checkpoint', checkpoint_file)[0]
        if old_folder!=new_folder:
            for key, path in checkpoint['path_helper'].items():
                checkpoint['path_helper'][key] = re.sub(r'logs/([^/]+)', 'logs/'+new_folder, path)
            torch.save(checkpoint, checkpoint_file)
        start_epoch = checkpoint['epoch']
        best_fid = checkpoint['best_fid']
        
        # dis_net.load_state_dict(checkpoint['dis_state_dict'])
        dis_net.load_state_dict(adapt_state_dict_for_loading(checkpoint['dis_state_dict'], dis_net))
        gen_optimizer.load_state_dict(adapt_state_dict_for_loading(checkpoint['gen_optimizer'], gen_optimizer))
        dis_optimizer.load_state_dict(adapt_state_dict_for_loading(checkpoint['dis_optimizer'], dis_optimizer))
        
        avg_gen_net = deepcopy(gen_net)
#         avg_gen_net.load_state_dict(checkpoint['avg_gen_state_dict'])
#         gen_net.load_state_dict(checkpoint['avg_gen_state_dict'])
        gen_avg_param = copy_params(checkpoint['avg_gen_state_dict'], mode='gpu')
        gen_net.load_state_dict(adapt_state_dict_for_loading(checkpoint['gen_state_dict'], gen_net))
...
#             train_d(args, gen_net, dis_net, dis_optimizer, train_loader, epoch, writer_dict,fixed_z, lr_schedulers)
        # TODO: shuffle train loader at each epoch
...
# Add module in model saving code exp'gen_net.module.state_dict()' to solve the model loading unpaired name problem
        has_attr_module = hasattr(gen_net, 'module')
        save_checkpoint({
            'epoch': epoch + 1,
            'gen_model': args.gen_model,
            'dis_model': args.dis_model,
            'gen_state_dict': gen_net.module.state_dict() if has_attr_module else gen_net.state_dict(),
            'dis_state_dict': dis_net.module.state_dict() if has_attr_module else dis_net.state_dict(),
            'avg_gen_state_dict': avg_gen_net.module.state_dict() if has_attr_module else avg_gen_net,
...
def gen_plot(gen_net, epoch, class_name):
    ...
    for i in range(2):
        for j in range(5):
            for data_pt in synthetic_data[i*5+j][0]:
                axs[i, j].plot(data_pt[0][:])
    buf = io.BytesIO()
